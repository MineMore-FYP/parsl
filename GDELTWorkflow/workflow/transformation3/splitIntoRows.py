...
df = pd.DataFrame()
for i in range(len(userScript3.orderOfModules)):
	#print(userScript3.orderOfModules[i])
	if currentModule == userScript3.orderOfModules[i]:
		if i == 0:
			df = pd.read_csv(userScript3.inputDataset)
			break
		else:
			previousModule = userScript3.orderOfModules[i-1]
			df = pd.read_csv(userScript3.outputLocation + previousModule + ".csv")
			break

outputDataset = userScript3.outputLocation + currentModule + ".csv"
#from userScript3 import userDefinedColumsToAggregate
